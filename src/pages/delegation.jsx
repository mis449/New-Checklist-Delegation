...
const CONFIG = {
  ...
  DRIVE_FOLDER_ID: "1Fk4-9cm0ceYpnry8Hli9CtcobRK4R7cB",
...
function DelegationDataPage() {
  ...
  // Role and username are now initialized directly from sessionStorage in useState hooks
  ...
  const sortDateWise = useCallback(
    (a, b) => {
      ...
      const statusA = a["col13"] || "";
      const statusB = b["col13"] || "";
  ...
  const uniqueNames = useMemo(() => {
    // If user is not admin, only return their own username in the list
    if (userRole !== "admin") {
      return [username];
    }

    // If admin and we have names from Whatsapp, use those
    if (whatsappNames && whatsappNames.length > 0) {
      return whatsappNames;
    }
  ...
  }, [accountData, historyData, userRole, username, whatsappNames]);
  ...
  const fetchSheetData = useCallback(async () => {
    try {
      ...
      const currentRole = userRole || sessionStorage.getItem("role");
      const currentUsername = username || sessionStorage.getItem("username");
      ...
      // Parallel fetch all sheets for better performance
      const [mainResponse, historyResponse, excelResponse] = await Promise.all([
        fetch(
        ...
        fetch(`${CONFIG.APPS_SCRIPT_URL}?sheet=Whatsapp&action=fetch`).catch(
          () => null
        ),
      ]);
      ...
      // Process Whatsapp names
      if (excelResponse && excelResponse.ok) {
        try {
          const excelText = await excelResponse.text();
          let excelData;
          try {
            excelData = JSON.parse(excelText);
          } catch (e) {
            const jsonStart = excelText.indexOf("{");
            const jsonEnd = excelText.lastIndexOf("}");
            excelData = JSON.parse(excelText.substring(jsonStart, jsonEnd + 1));
          }

          let excelRows =
            excelData.table?.rows ||
            excelData.values?.map((row) => ({
              c: row.map((val) => ({ v: val })),
            })) ||
            excelData;

          const namesMaster = excelRows
            .map((row, idx) => {
              if (idx === 0) return null;
              let rowValues = row.c ? row.c.map((cell) => cell?.v || "") : row;
              return rowValues[2]; // Column C (index 2) is Username
            })
        ...
        try {
          ...
          if (historyData && historyData.table && historyData.table.rows) {
            processedHistoryData = historyData.table.rows
              ...
              .filter((row) => row !== null && (row.col1 || row.col8));
          }
        } catch (historyError) {
      ...
      const allExtraHistoryData = [];
      const allSourceDataForMatching = [];
      ...
      rows.forEach((row, rowIndex) => {
        ...
        // Skip rows that are empty or don't have essential task info
        ...
        for (let i = 0; i < 21; i++) {
          if (i === 0 || i === 6 || i === 10) {
            ...
            // Column N (13) is Status - Default to "Pending" if empty
            rowData[`col${i}`] = rowValues[i] || "Pending";
          } else {
            rowData[`col${i}`] = rowValues[i] || "";
          }
        }
        ...
        if (isCompleted) {
          ...
          const historyVersion = {
            ...
            col5: "", // Image
            col7: rowData.col4, // Name
            col8: rowData.col5, // Task Description
            col9: rowData.col3  // Given By
          };
          allExtraHistoryData.push(historyVersion);
        } else {
          allPendingData.push(rowData);
        }
      });
    ...
    (e) => {
      ...
      if (checked) {
        ...
        filteredAccountData.forEach((item) => {
          if (!isTaskDisabled(item["col13"], userRole)) {
                ...
                <div className="min-w-full">
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      ...
                      ) : !loading && (searchTerm || startDate || endDate || nameFilter || historyData.length === 0) ? (
                        <tr>
                          <td
                            colSpan={userRole === "admin" ? 12 : 9}
                            className="px-6 py-4 text-center text-gray-500"
                          >
                            {searchTerm || startDate || endDate || nameFilter
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex items-center space-x-3">
                              <input
                                ...
                                disabled={isTaskDisabled(account["col13"], userRole)}
                              />
                              <div className="flex flex-col">
                                <span className={`px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(account["col13"] || "Pending")}`}>
                                  {account["col13"] || "Pending"}
                                </span>
                          ...
                          <input
                            ...
                            checked={
                              ...
                              filteredAccountData.filter(
                                (item) => !isTaskDisabled(item["col13"], userRole)
                              ).length > 0 &&
                              filteredAccountData.filter(
                                (item) =>
                                  !isTaskDisabled(item["col13"], userRole) &&
                          ...
                          return (
                            <tr
                              ...
                              className={`${isSelected ? "bg-purple-50" : ""
                                } hover:bg-gray-50 ${rowColorClass} ${isTaskDisabled(account["col13"], userRole)
                            ...
                            >
                              ...
                              <td className={`px-6 py-4 whitespace-nowrap ${rowColorClass}`}>
                                <span
                              ...
                              >
                                <input
                                  ...
                                  disabled={
                                    !isSelected ||
                                    isTaskDisabled(account["col13"], userRole)
                                  }
                                  value={remarksData[account._id] || ""}
                                  onChange={(e) =>
                                    setRemarksData((prev) => ({
