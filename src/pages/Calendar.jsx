...
const CalendarUI = ({ userRole, userName, displayName }) => {
  ...
  // --- Data transform for different sheets ---
  const transformToTasks = (rows, sheetType = "Checklist") => {
    if (!rows || rows.length === 0) return [];
    let tasks = [];

    // Indices (based on Checklist/Delegation structure):
    // 0: Timestamp, 1: Task ID, 2: Department, 3: Given By, 4: Name,
    // 5: Description, 6: Start Date, 7: Frequency, 9: Time (for Checklist),
    // 10: Actual/Done (for Checklist), 15: Admin Done (for Checklist)
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      // Determine time index (Checklist usually index 9)
      const timeStr = c[9]?.v || "";
      
      // Determine status
      const actualVal = c[10]?.v || "";
      const adminDoneVal = c[15]?.v || "";
      const isDone = (actualVal && actualVal.toString().trim() !== "") || 
                     (adminDoneVal && adminDoneVal.toString().trim() === "Admin Done");
      ...
      tasks.push({
        ...
        time: timeStr,
        status: status,
        remarks: c[12]?.v || "", // Remarks usually at index 12
        priority: c[17]?.v || "normal", // Delegation priority at 17, or index 11 in others?
        timestamp: c[0]?.v || "",
        rowIndex: i + 2,
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      let checklistTasks = [];
      if (checklistResponse.data?.table?.rows) {
        checklistTasks = transformToTasks(checklistResponse.data.table.rows, "Checklist");
      }

      let delegationTasks = [];
      if (delegationResponse.data?.table?.rows) {
        delegationTasks = transformToTasks(delegationResponse.data.table.rows, "Delegation");
      }
      ...
      Object.keys(map).forEach((dateStr) => {
        ...
        Object.keys(tasksByTime).forEach((timeKey) => {
          ...
          eventsArray.push({
            ...
            backgroundColor: tasksAtTime[0].sheetType === "DELEGATION" ? sheetColors.DELEGATION : sheetColors.Checklist,
          });
        });
      });
