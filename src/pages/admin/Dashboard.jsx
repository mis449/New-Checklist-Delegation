...
export default function AdminDashboard() {
  const APPS_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbw5Vs4zmDWa4MPT1_nl5s-DHXoFXp8Wfdnq067gCDArNOrunhetwt8CUj7ApAcohGR2sw/exec";
    ...
    try {
      ...
      const uploadResponse = await fetch(APPS_SCRIPT_URL, {
        ...
        body: new URLSearchParams({
          ...
          folderId: '1Fk4-9cm0ceYpnry8Hli9CtcobRK4R7cB', // Your specified folder ID
          username: username
        })
      });
  ...
  const getIsGracePeriodUser = (name) => {
    if (!name) return false;
    return usersWithGracePeriod.includes(name.trim().toUpperCase());
  };
  ...
  const fetchDepartmentData = async () => {
    ...
    try {
      // Fetch task data and Whatsapp sheet in parallel
      const [response, whatsappResponse] = await Promise.all([
        fetch(`${APPS_SCRIPT_URL}?action=fetch&sheet=${sheetName}`),
        fetch(`${APPS_SCRIPT_URL}?action=fetch&sheet=Whatsapp`).catch(() => null)
      ]);
      ...
      // Process Whatsapp names if available
      if (whatsappResponse && whatsappResponse.ok) {
        try {
          const whatsappData = await whatsappResponse.json();
          const names = whatsappData.table.rows
            .map((row, index) => {
              if (index === 0) return null;
              return getCellValue(row, 2); // Column C (index 2) - Username
            })
            .filter(name => name && name !== "" && name !== "-");

          setWhatsappNames(Array.from(new Set(names)).sort());
        } catch (e) {
          console.error("Error processing Whatsapp names:", e);
        }
      }
      ...
      const processedRows = data.table.rows
        .map((row, rowIndex) => {
          ...
          const leaveValSource = getCellValue(row, 16);
          const isLeave = leaveValSource && String(leaveValSource).trim().toLowerCase().includes("leave");
          ...
          let status = "pending";
          const adminDoneVal = getCellValue(row, 15);
          const isAdminDone = adminDoneVal && String(adminDoneVal).trim() === "Admin Done";

          if (dashboardType === "delegation") {
            const isDone = statusColumnU === "Done" || (completionDate && completionDate !== "");
            if (isDone) status = "completed";
            else if (isDateInPast(taskStartDate) && !isDateToday(taskStartDate)) {
              // Delegation might not have grace period, or use same? Let's use standard for now
              status = "overdue";
            }
          } else {
            const isGraceUser = getIsGracePeriodUser(assignedTo);
            const isPast = isDateInPast(taskStartDate) && !isDateToday(taskStartDate);
            const isActuallyOverdue = isPast && !(isGraceUser && isYesterday(taskStartDate));

            if (isAdminDone || (completionDate && completionDate !== "")) status = "completed";
            else if (isActuallyOverdue) status = "overdue";
            else status = "pending";
          }
          ...
          // UPDATED: Count for dashboard cards
          if (dashboardType === "delegation") {
            totalTasks++;
            ...
            if (status === "completed") {
              ...
              // Count by rating from Column R (index 17)
              const ratingNum = Number(ratingColumnR);
              if (ratingNum === 1) completedRatingOne++;
              else if (ratingNum === 2) completedRatingTwo++;
              else if (ratingNum >= 3) completedRatingThreePlus++;
            ...
            // Checklist mode - Count all tasks to match sheet data
            totalTasks++;
            ...
            if (status === "completed") {
              ...
              const completedMonth = parseDateFromDDMMYYYY(completionDate) || today;
              const monthName = completedMonth.toLocaleString("default", { month: "short" });
              if (monthlyData[monthName]) monthlyData[monthName].completed++;
            } else {
              staffData.pendingTasks++;
              pendingTasks++; // Card shows all non-completed

              if (status === "overdue") {
                overdueTasks++;
                statusData.Overdue++; // Chart uses exclusive categories
              } else {
          ...
          <div className="rounded-lg border border-l-4 border-l-green-500 shadow-md hover:shadow-lg transition-all bg-white">
            ...
            <div className="p-4">
              ...
              <p className="text-xs text-green-600">
                {dashboardType === "delegation"
                  ? "Tasks marked Done or with Actual date"
              ...
              <div className="space-y-2 md:w-[180px]">
                ...
                <CustomDropdown
                  ...
                  options={[
                    ...
                    ...(whatsappNames.length > 0
                      ? (isAdminUser()
                        ? whatsappNames
                        : whatsappNames.filter(name => name.toLowerCase() === sessionStorage.getItem("username")?.toLowerCase())
