...
const getTaskCategory = (startDateStr) => {
  const taskDate = parseDateFromDDMMYYYY(startDateStr)
  if (!taskDate) return ""
...
const isLeaveStatus = (leaveValue) => {
  return !isEmpty(leaveValue) && leaveValue.toString().trim().toLowerCase().includes("leave");
};
    ...
    >
      ...
      <td className={`px-3 py-4 ${isNotToday ? "bg-white border-l-4 border-white" : "hover:bg-gray-50"} min-w-[100px]`}>
        <select
          ...
          className={`border border-gray-300 rounded-md px-2 py-1 w-full text-black disabled:bg-gray-100 disabled:cursor-not-allowed text-sm font-medium ${isDisabled ? "opacity-50" : ""}`}
        >
      ...
      <td className={`px-3 py-4 ${isNotToday ? "bg-white border-l-4 border-white" : "hover:bg-gray-50"} min-w-[150px]`}>
        <input
          ...
          className={`border rounded-md px-2 py-1 w-full border-gray-300 text-gray-900 placeholder-gray-400 disabled:bg-gray-100 disabled:cursor-not-allowed text-sm break-words font-medium ${isDisabled ? "opacity-50" : ""}`}
        />
      </td>
      ...
      <td className="px-3 py-4 min-w-[80px]">
        <div className={`text-sm break-words font-medium text-gray-900 ${isDisabled ? "opacity-50" : ""}`}>
          {account["col7"] || "—"}
        </div>
      </td>
...
function AccountDataPage() {
  ...
  const confirmReassign = async () => {
    ...
    try {
      // Prepare data for updateTaskData (Column K/Actual and others)
      const taskData = selectedHistoryItems.map((item) => ({
        taskId: item._taskId || item["col1"],
        rowIndex: item._rowIndex,
        actualDate: " ", // Use space to clear
        status: " ",
        remarks: " ",
        imageUrl: " "
      }))

      // Prepare data for updateAdminDone (Column P)
      const adminData = selectedHistoryItems.map((item) => ({
        taskId: item._taskId || item["col1"],
        rowIndex: item._rowIndex,
        adminDoneStatus: " " // Clear it
      }))

      const formData1 = new FormData()
      formData1.append("sheetName", CONFIG.SHEET_NAME)
      formData1.append("action", "updateTaskData")
      formData1.append("rowData", JSON.stringify(taskData))

      const formData2 = new FormData()
      formData2.append("sheetName", CONFIG.SHEET_NAME)
      formData2.append("action", "updateAdminDone")
      formData2.append("rowData", JSON.stringify(adminData))

      // Execute both updates
      const [res1, res2] = await Promise.all([
        fetch(CONFIG.APPS_SCRIPT_URL, { method: "POST", body: formData1 }),
        fetch(CONFIG.APPS_SCRIPT_URL, { method: "POST", body: formData2 })
      ])
      ...
      if (result1.success && result2.success) {
        ...
        setSelectedHistoryItems([])
        setSuccessMessage(`Successfully re-assigned ${selectedHistoryItems.length} items!`)
        setTimeout(() => fetchSheetData(), 2000)
      } else {
        throw new Error(result1.error || result2.error || "Failed to re-assign")
      }
    } catch (error) {
